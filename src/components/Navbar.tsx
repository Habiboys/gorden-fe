  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-24">
            ...
            {menuItems.map((item) => {
              ...
              return isHash ? (
                <a
                  key={item.name}
                  href={item.path}
                  className={`text-sm tracking-wide transition-colors relative group py-2 ${
                    isSolidNav 
          ...
          <div className="flex items-center gap-2">
            <Button 
              variant="ghost" 
              size="icon" 
              className={`hover:bg-transparent relative group transition-colors ${
                isSolidNav 
            ...
            <div className={`w-px h-6 mx-2 hidden lg:block transition-colors ${
              isSolidNav ? 'bg-gray-200' : 'bg-white/20'
            }`} />
            <Link to="/login">
              <Button
                variant="outline"
                className={`hidden lg:flex px-5 py-2 rounded-full text-sm transition-all ${
                  isSolidNav 
            ...
            <Link to="/register">
              <Button
                className={`hidden lg:flex px-6 py-2 rounded-full text-sm transition-all ${
                  isSolidNav 
            ...
            <Button
              ...
              className={`lg:hidden transition-colors ${
                isSolidNav ? 'text-gray-900' : 'text-white'
              }`}
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className={`lg:hidden py-6 border-t transition-colors ${
            isSolidNav ? 'border-gray-100 bg-white' : 'border-white/10 bg-black/20 backdrop-blur-xl'
          }`}>
              ...
              {menuItems.map((item) => {
                ...
                return isHash ? (
                  <a
                    key={item.name}
                    href={item.path}
                    className={`block py-3 transition-colors border-l-2 border-transparent pl-4 ${
                      isSolidNav 
                  ...
                  <Link
                    key={item.name}
                    to={item.path}
                    className={`block py-3 transition-colors border-l-2 pl-4 ${
                      isActive ? 'border-[#EB216A]' : 'border-transparent'
                    } ${
                      isSolidNav 
                        ? isActive
                          ? 'text-[#EB216A]'
